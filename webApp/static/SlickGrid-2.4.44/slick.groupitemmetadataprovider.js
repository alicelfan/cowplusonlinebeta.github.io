(function ($) {
  ...
  function GroupItemMetadataProvider(inputOptions) {
    ...
    function onExpandClick(e, args) {
        ...
        if((item.groupingKey.includes("Country-Year Data"))||item.groupingKey.includes("Dyad-Year Data")){
        ...
        if((item.groupingKey.includes("Country-Year Data")) && ((expanded_groups.includes("Preloaded Datasets:|:Dyad-Year Data")) || (expanded_groups.includes("Uploaded Datasets:|:Dyad-Year Data")))){
          this.getData().collapseGroup("Preloaded Datasets:|:Dyad-Year Data");
          this.getData().collapseGroup("Uploaded Datasets:|:Dyad-Year Data");
          this.setSelectedRows([]);
          index = expanded_groups.indexOf("Preloaded Datasets:|:Dyad-Year Data");
          if (index > -1) { // only splice array when item is found
            expanded_groups.splice(index, 1); // 2nd parameter means remove one item only
          }
          index = expanded_groups.indexOf("Uploaded Datasets:|:Dyad-Year Data");
          if (index > -1) { // only splice array when item is found
            expanded_groups.splice(index, 1); // 2nd parameter means remove one item only
          }
        }
        if((item.groupingKey.includes("Dyad-Year Data")) && ((expanded_groups.includes("Preloaded Datasets:|:Country-Year Data")) || (expanded_groups.includes("Uploaded Datasets:|:Country-Year Data")))){
          this.getData().collapseGroup("Preloaded Datasets:|:Country-Year Data");
          this.getData().collapseGroup("Uploaded Datasets:|:Country-Year Data");
          this.setSelectedRows([]);
          index = expanded_groups.indexOf("Preloaded Datasets:|:Country-Year Data");
          if (index > -1) { // only splice array when item is found
            expanded_groups.splice(index, 1); // 2nd parameter means remove one item only
          }
          index = expanded_groups.indexOf("Uploaded Datasets:|:Country-Year Data");
          if (index > -1) { // only splice array when item is found
            expanded_groups.splice(index, 1); // 2nd parameter means remove one item only
          }
        }
